...
database=input("Type enter database name :\n")
...
def Display_Choices():
    ...
    print("Enter 10 : Is BCNF ?")
    print("Enter 11 : Is 3NF ?")
...
while value != "0":
    if value == "1":
            ...
            for y in range(0, len(x)+1):
                for z in itertools.combinations(x, y):
                    ...
                    while(again==True):
                        ...
                        for row in cur.execute("SELECT * FROM FuncDep"):
                            if row[1] in z_new and row[0]==table_name:
        ...
        table_name2 = Ask_Existing_Table()
        count=0
        for row in cur.execute("SELECT * FROM FuncDep"):
            if row[0]==table_name2:
                count+=1
        if count==0:
            print("False")
        else:
            cur.execute('SELECT * from '+table_name2)
            x2=[description[0] for description in cur.description]
            keys2=[]
            for y2 in range(0, len(x2)+1):
                for z2 in itertools.combinations(x2, y2):
                    z2_new=copy.deepcopy(list(z2))
                    again2=True
                    while(again2==True):
                        z2_new_new=copy.deepcopy(list(z2_new))
                        for row in cur.execute("SELECT * FROM FuncDep"):
                            if row[1] in z2_new and row[0]==table_name2:
                                if row[2] not in z2_new:
                                    z2_new.append(row[2])
                        if z2_new_new==z2_new:
                            again2=False
                        if sorted(z2_new)==sorted(x2):
                            if len(keys2)>0:
                                if len(z2)==len(keys2[0]):
                                    keys2.append(z2)
                            else:
                                keys2.append(z2)
                            again2=False
            ...
            for row in cur.execute("SELECT * FROM FuncDep"):
                if row[0]==table_name2:
                    ...
                    for key in keys2:
                        key_bis=str(key)
                        size=len(key_bis)
                        if key_bis[2:size-3] in row[1]:
                            key_ok=True
                    if key_ok==False:
                        BCNF=False
            if BCNF:
                print("This table is in BCNF")
            else :
                print("This table is not in BCNF")
        ...
        if count==0:
            ...
            if BCNF==False:
                if NF:
                    ...
                    print("This table is not in 3NF")
